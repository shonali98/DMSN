...
function IssueInventoryView({navigation,route}) {
  ...
  const parseCustomDate = (dateString) => {
    ...
    const handleSearch = async() => {
      ...
      console.log("dsr",selectedValue)
      console.log("dsrdetails",dsrDetails)
      console.log("data",data)
      const selectedDSRName = selectedValue;

      // Find the DSR details based on the selected DSR name
      const selectedDSR = dsrDetails.find(dsr => dsr.bisName === selectedDSRName);
      let bisId;
      // Check if the DSR is found and extract bisId
      if (selectedDSR) {
          bisId = selectedDSR.bisId;
          console.log('bisId:', bisId);
      } else {
          console.log('DSR not found in dsrDetails.');
      }
      ...
      try {
        ...
        const apiUrl = `http://203.189.68.156:8181/SingerPortalWebService-4.2/Services/IssueInventories/getInventoryIssueList?status=${optionStatus}&dsrId=${bisId}&distributerID=${data.data[0].extraParams}`;
      ...
      if (data && data.data && data.data.length > 0) {
        ...
        navigation.navigate('IssueInventoryViewSearch',{data:data,displayData: filteredData,selectedOption:selectedOption})
...
{/* {item.additionalData && (
  ...
  const handleGetDsrSub = async () => {
    try {
      const apiUrl = `http://dmsn.lk:8282/SingerPortalWebService-4.1/Services/UserTypeLists/getUserTypeListLowUpper?userId=${data.data[0].userId}`;
...
export default IssueInventoryView;
